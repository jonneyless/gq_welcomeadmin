...
<div class="box box-primary">
    <div class="box-body">
        <div class="mailbox-controls pull-left">
            <span style="color: gray;margin-top: 5px;">用户的<span style="color: red; font-weight: bold;">简介</span>中包含限制词，则按等级进行操作</span>
...
<script>
  $(function () {
    let dataTable = $('#dataTable').DataTable({
      ...
      "columns": [{"data": "name"}, {"data": "level"}, {"data": "id"},],
    ...
    $('#dataTable').on("draw.dt", function () {
      $("tr").each(function () {
        $(this).find(".level").click(function () {
          ...
          $.ajax({
            url: "{{adminUrl('word/change/level')}}", type: "post", data: {
              _token: '{{csrf_token()}}', "id": word_id, "level": level, "type": 10,
        ...
        $(this).find(".delete").click(function () {
          ...
          $.ajax({
            url: "{{adminUrl('word/delete')}}", type: "post", data: {
              _token: '{{csrf_token()}}', "id": word_id, "type": 10,
            }, success: function (data) {
              layer.close(index_load);
    ...
    $(".add").click(function () {
      let layer1 = layer.open({
        ...
        content: "<div style='text-align: center;' class='panel_add'><div style='margin-top: 15px;margin-bottom: 10px;'>限制词：<input type='text' id='in' class='word_data'></div>" + "<div style='margin-top: 10px;margin-bottom: 10px;'>限制等级：一级 &nbsp;<input type='radio' name='word_level' value='1' checked>&nbsp;&nbsp;&nbsp;\n" + "二级 &nbsp;<input type='radio' name='word_level' value='2'>&nbsp;&nbsp;&nbsp;\n" + "</div>" + "<button type='button' class='btn btn-info btn-sm sure'>确认</button></div>",
      });
      $("body").on("click", ".sure", function () {
        ...
        $.ajax({
          url: "{{adminUrl('word/add')}}", type: "post", data: {
            _token: '{{csrf_token()}}', "name": word_data, "level": word_level, "type": 10,
          }, success: function (data) {
            layer.close(layer1);
            layer.msg(data["message"], {time: 200});
            document.location.reload();
