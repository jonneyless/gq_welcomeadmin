...
<div class="box box-primary">
    <div class="box-body">
        <div class="mailbox-controls pull-left">
            <span style="color: gray;margin-top: 5px;">用户<span style="color: red; font-weight: bold;">昵称</span>中包含限制词，则按等级进行操作</span>
            ...
            <div class="blue">
                <p>正则录入格式：引(.*)流</p>
                <p>匹配数据：引流，引1流，引a流，引+流，引1234流，引abcd流，引+-()流</p>
            </div>
        </div>
...
<script>
    $(function () {
        let dataTable = $('#dataTable').DataTable({
            ...
            "ajax": {
                "method": "post",
                "url": "<?php echo e(adminUrl('word/data')); ?>",
                "data": function (d) {
                    return $.extend({}, d, {
        ...
        $('#dataTable').on("draw.dt", function () {
            $("tr").each(function () {
                $(this).find(".level").click(function () {
                    ...
                    $.ajax({
                        url: "<?php echo e(adminUrl('word/change/level')); ?>",
                        type: "post",
                        data: {
                            _token: '<?php echo e(csrf_token()); ?>',
                            "id": word_id,
                ...
                $(this).find(".delete").click(function () {
                    ...
                    let word_id = $(this).attr("word_id");
                    $.ajax({
                        url: "<?php echo e(adminUrl('word/delete')); ?>",
                        type: "post",
                        data: {
                            _token: '<?php echo e(csrf_token()); ?>',
                            "id": word_id,
        ...
        $(".add").click(function () {
            ...
            $("body").on("click", ".sure", function () {
                ...
                $.ajax({
                    url: "<?php echo e(adminUrl('word/add')); ?>",
                    type: "post",
                    data: {
                        _token: '<?php echo e(csrf_token()); ?>',
                        "name": word_data,
